...
//echo "here";
//echo $query;
            //              print iconv("windows-1251","winodws-1251",$sql);
            // $query = iconv( "utf-8", "windows-1251", $sql );
// wolfdebug                print $sql;
            if ( empty($query) ) {
                $query =$sql;
            }
            return mysql_query($query,$this->_connectionID);
    ...
    class ADORecordSet_mysql extends ADORecordSet{
        ...
        function _fetch()
        {
            ...
            if ( is_array($this->fields ) ) {
                ...
                $types = $this->FieldTypesArray();
                foreach ( $types as $type ) {
                    if ( ($type->type == 'string' || $type->type =='blob')
                        && !empty($this->fields[ $type->name ])
                        && $this->fields[ $type->name ]!='NULL'
                    ) {
                        //print $type->name." - ".$this->fields[ $type->name ];
                        /*
                        if ( $type->name == 'expert' ) {
                                print $type->name.' '.$this->fields[ $type->name ].' - '.$type->type."<br/>";
                                print mb_detect_encoding( $this->fields[ $type->name ] );
                        }*/
//                                        if ( mb_detect_encoding( $this->fields[ $type->name ] ) != 'UTF-8' ) {
                        // $this->fields[ $type->name ] = $this->fields[ $type->name ]
                        //        iconv( "windows-1251", "utf-8", $this->fields[ $type->name ] );
//                                        }
                        /*
                        if ( $type->name == 'expert' ) {
                                print '2'.$type->name.' '.$this->fields[ $type->name ].' - '.$type->type."<br/>";
                        }*/
                        //print " - ".$this->fields[ $type->name ]."<br>\n";
                    } else {
                    }
                }
            }
